...
export function buildRunSteps(g){return [
 {phase:'Prepare',side:'admin',action:`Before sharing, open the administrator browser profile at entra.microsoft.com. In the ${g.liveUrl?'customer application':'user desktop'}, prepare ${g.account}. Keep the relevant views ready on both sides.`,why:`We will follow ${g.name} through ${g.title.toLowerCase()} and compare the controls with the experience.`,look:`Confirm the actual account and tenant in both browsers. ${g.liveUrl?'Rehearse the customer sign-in and protected action':'Test typing'}, complete MFA privately, and check the scenario’s starting state.`},
