...
export function installDualStage(root,config,validateGateway,onUserFocus){
 ...
 let frame=null,stopFocusWatch=null,timer=null,layout='both',url=null;
 ...
 function focusUser(){if(!frame)return;onUserFocus();focusRemoteWindow(frame);}
 ...
 function connect(){if(frame||!url)return;frame=document.createElement('iframe');frame.id='user-session-frame';frame.title='Protected independent kiosk desktop for the demo persona';frame.tabIndex=0;stopFocusWatch=watchRemoteFocus(frame,onUserFocus);frame.referrerPolicy='no-referrer';frame.setAttribute('allow','fullscreen');frame.setAttribute('allowfullscreen','');frame.src=url;welcome.hidden=true;focus.disabled=false;disconnect.disabled=false;status.textContent='Opening protected gateway…';frame.addEventListener('load',()=>{clearTimeout(timer);status.textContent='Gateway page opened · sign in inside';if(document.activeElement===frame&&layout!=='admin')focusUser();});frame.addEventListener('error',()=>{clearTimeout(timer);status.textContent='Gateway unavailable · try separate tab';});user.querySelector('.user-display').append(frame);timer=setTimeout(()=>status.textContent='Still waiting · try separate tab',20000);focusUser();}
